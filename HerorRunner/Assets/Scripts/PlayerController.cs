...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!isRotating)
        {
            ...
            if (Physics.Raycast(cPosition, transform.forward, 1.5f) && leftWall && rightWall)
            {
                isMoving = false;
                transform.DOLocalRotate(transform.up * (transform.rotation.eulerAngles.y - 180f),
                    turnSpeed*2).OnComplete(delegate() { isRotating = false;
                    searchForCenter = true;
                    isMoving = true;
                });
            }
